...
class demoDataReader : public DataReader 
{
public:
	demoDataReader(DataReaderImpl* pDataReaderImpl);

	~demoDataReader();

	static demoDataReader* narrow(DataReader* pDataReader);

	ReturnCode_t take(demoSeq& data_values, SampleInfoSeq & sample_infos, int max_samples, SampleStateKind sample_states, ViewStateKind view_states, InstanceStateKind instance_states);

	ReturnCode_t read(demoSeq& data_values, SampleInfoSeq & sample_infos, int max_samples, SampleStateKind sample_states, ViewStateKind view_states, InstanceStateKind instance_states);

	ReturnCode_t take_next_sample(demo& receivedData, SampleInfo & sample_info);

	ReturnCode_t return_loan(demoSeq& data_values, SampleInfoSeq& sample_infos);
