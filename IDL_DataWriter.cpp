...
demoDataWriter::demoDataWriter(DataWriterImpl * pDataWriterImpl)
	: DataWriter(pDataWriterImpl)
{
}

demoDataWriter::~demoDataWriter()
{
}

demoDataWriter* demoDataWriter::narrow(DataWriter* pDataWriter) 
{
	if (!pDataWriter)
		return NULL;
	demoDataWriter *ademoDataWriter = (demoDataWriter*)pDataWriter;
    return ademoDataWriter;
}

ReturnCode_t demoDataWriter::write(demo& data, const InstanceHandle_t & handle){
	char *buf = new char[DEFAULT_CDRSIZE];
	CDR* CDRData = NULL;
	ReturnCode_t ret = RETCODE_OK;
	CDRData = new CDR((unsigned char *)buf, DEFAULT_CDRSIZE, MY_BYTE_SEX, 1);
