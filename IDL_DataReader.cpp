...
demoDataReader::demoDataReader(DataReaderImpl * pDataReaderImpl)
	:DataReader(pDataReaderImpl)
{
}
...
demoDataReader* demoDataReader::narrow(DataReader* pDataReader) 
{
	if (!pDataReader)
		return NULL;

	demoDataReader *ademoDataReader = (demoDataReader*)pDataReader;
	return ademoDataReader;
}

ReturnCode_t demoDataReader::take(demoSeq & data_values, SampleInfoSeq & sample_infos, int max_samples, SampleStateKind sample_states, ViewStateKind view_states, InstanceStateKind instance_states)
{
	UserDataVector userDataVector;
	DataReader::take(userDataVector, sample_infos, max_samples, sample_states, view_states, instance_states);
...
ReturnCode_t demoDataReader::read(demoSeq & data_values, SampleInfoSeq & sample_infos, int max_samples, SampleStateKind sample_states, ViewStateKind view_states, InstanceStateKind instance_states)
{
	UserDataVector userDataVector;
	DataReader::read(userDataVector, sample_infos, max_samples, sample_states, view_states, instance_states);
...
ReturnCode_t demoDataReader::take_next_sample(demo& receivedData, SampleInfo& sample_info)
{
	UserData userData;
	DataReader::take_next_sample(userData, sample_info);
	CDR* CDRData = new CDR((unsigned char *)userData.pData + 4, userData.ilength - 4, MY_BYTE_SEX, 1);
...
ReturnCode_t demoDataReader::return_loan(demoSeq& data_values, SampleInfoSeq& sample_infos)
{
	return ReturnCode_t();
}
